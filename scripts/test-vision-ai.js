...
async function convertPdfPageToImage(pdfPath, pageNum = 0) {
  ...
  try {
    ...
    throw new Error(`Failed to convert PDF to image: ${error.message || String(error)}`);
  }
}
...
async function extractTextFromPdf(filePath) {
  ...
  try {
    ...
    try {
      ...
      console.warn('Could not determine PDF page count. Processing only first page.');
    }
...
async function testVisionAI() {
  try {
    ...
    if (!credentialsPath) {
      ...
      console.error('Please set it to the path of your Google Cloud credentials JSON file');
      process.exit(1);
    }

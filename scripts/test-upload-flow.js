...
async function testUploadFlow() {
  try {
    ...
    console.log(`📄 File: ${fileName} (${fileType}, ${(fileStats.size / 1024).toFixed(2)} KB)`);
    ...
    try {
      ...
      if (!uploadResponse.ok) {
        ...
        throw new Error(`API responded with status ${uploadResponse.status}: ${errorData}`);
      }
      ...
      try {
        ...
        const extractedText = await processStatement(uploadResult.path, fileType);
      ...
      console.log(`📊 Statement count before: ${beforeCount}, after: ${afterCount}`);
      
      if (afterCount !== beforeCount + 1) {
        throw new Error('Statement count mismatch. Record may not have been created properly.');
      }
      ...
      console.log(`📅 Processed timestamp: ${finalStatement.processedTimestamp}`);
      console.log(`📊 Raw text length: ${finalStatement.rawText ? finalStatement.rawText.length : 0} characters`);

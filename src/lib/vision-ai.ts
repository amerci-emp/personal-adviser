...
export function initVisionClient(): ImageAnnotatorClient {
  if (!visionClient) {
    try {
      ...
      console.log(
        `Initializing Vision AI client with credentials at: ${credentialsPath}`,
      );
      ...
      try {
        ...
        fs.access(fullPath).catch((err) => {
          throw new Error(
            `Credentials file not found at ${fullPath}: ${err.message}`,
          );
        });
      } catch (error) {
...
export async function extractTextFromImage(filePath: string): Promise<string> {
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        `Image file not found at ${filePath}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
...
async function convertPdfPageToImage(
  ...
  try {
    ...
    try {
      ...
      await execAsync(
        `convert -density 300 -quality 100 "${pdfPath}[${pageNum}]" "${outputPath}"`,
      );
      console.log(`PDF page converted to: ${outputPath} using ImageMagick`);
      return outputPath;
    } catch (convertError: any) {
      console.warn(
        "ImageMagick convert failed, trying with alternate approach:",

...
async function saveToGoogleSheets(userId: string, transactions: any[]) {
  console.log(`[MOCK] Saving ${transactions.length} transactions to Google Sheets for user ${userId}`);
...
async function cleanupTemporaryFiles(filePath: string) {
  try {
...
export const statementRouter = createTRPCRouter({
  ...
  upload: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      console.log(`Statement upload started: ${input.filename} (${input.fileType})`);
      ...
      try {
        ...
        const processingResult = await processUploadedFile(input.fileUrl, input.fileType);
        
        if (!processingResult.success) {
          console.error(`OCR processing failed: ${processingResult.error}`);
          throw new Error(processingResult.error || "Failed to process statement");
        }
